...
class Untwister:
    ...
    def symbolic_untamper(self, solver, y):
        ...
        # coefficients come from wikipedia
        equations = [
            y2 == y1 ^ (LShR(y1, 11)),
    ...
    def get_symbolic(self, guess):
        name = next(SYMBOLIC_COUNTER)
        ERROR = 'Must pass a string like "?1100???1001000??0?100??" where ? represents an unknown bit'
    ...
    def submit(self, guess):
        '''
        we need 624 number to completely clone the state.
        '''
        if self.index >= 624:
            name = next(SYMBOLIC_COUNTER)
